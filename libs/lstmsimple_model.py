...
class LSTM(tf.keras.Model):
  ...
  def compile(self, lr=0.0002):
    super(LSTM, self).compile(metrics = [self.PSNR])
  ...
  def build_pconv_unet(self, train_bn=True):      
    ...
    lstm_layer = tf.keras.layers.LSTM(self.img_cols, return_sequences=True)
    lstm_outputs = lstm_layer(inputs_boolm)[:,-int(self.img_rows*self.mask_ratio):]
    #
    outputs = tf.concat([inputs_boolm, lstm_outputs], axis=1)
