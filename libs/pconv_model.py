...
from libs.assign_lib import BatchAssignOneh, BatchAssignProb

class PConvUnet(tf.keras.Model):
  def __init__(self, img_rows=512, img_cols=512, c_dim=3,
    ...
    super(PConvUnet, self).__init__(*args, **kwargs)
  ...
  def compile(self, lr=0.0002):
    # print('compiling')
    super(PConvUnet, self).compile(metrics = [self.PSNR])
    # print('compiled5', self._is_compiled)
    # print('compiled5b', self.model._is_compiled)
    self.optimizer = Adam(learning_rate=lr)
    self.loss = self.loss_total()
    # print('compiled6', self._is_compiled)
  ...
  def build_pconv_unet(self, train_bn=True):      
      ...
      # inputs_img, inputs_mask = tf.keras.layers.Lambda(
      #     lambda x: tf.unstack(x, axis=-1)
      # )(inputs_img_mask)
      ...
      if np.log2(self.img_rows)-int(np.log2(self.img_rows))!=0 or np.log2(self.img_cols)-int(np.log2(self.img_cols))!=0: # dimensions are not a power of 2
          ...
          pad_r1 = int((2 ** r_pow2 - self.img_rows) // 2)
          pad_r2 = int((2 ** r_pow2 - self.img_rows) - ((2 ** r_pow2 - self.img_rows) // 2))
          c_pow2 = max(8,np.ceil(np.log2(self.img_cols)))
          pad_c1 = int((2 ** c_pow2 - self.img_cols) // 2)
          pad_c2 = int((2 ** c_pow2 - self.img_cols) - ((2 ** c_pow2 - self.img_cols) // 2))
      ...
      if self.c_dim == 1:
          inputs_img_P = Lambda(lambda x: tf.tile(x, [1, 1, 1, 3]))(inputs_img_P)
          inputs_mask_P = Lambda(lambda x: tf.tile(x, [1, 1, 1, 3]))(inputs_mask_P)
      
      # ENCODER
      def encoder_layer(img_in, mask_in, filters, kernel_size, bn=True):
          conv, mask = PConv2D(filters, kernel_size, strides=2, padding='same', 
                               name = 'EncPConv2D'+str(encoder_layer.counter))(
                                   [img_in, mask_in])
          if bn:
              conv = BatchNormalization(name='EncBN'+str(encoder_layer.counter))(conv, training=train_bn)
          conv = Activation('relu', name='EncReLu'+str(encoder_layer.counter))(conv)
          encoder_layer.counter += 1
          return conv, mask
      encoder_layer.counter = 0
      
      e_conv1, e_mask1 = encoder_layer(inputs_img_P, inputs_mask_P, 64, 7, bn=False)
      ...
      def decoder_layer(img_in, mask_in, e_conv, e_mask, filters, kernel_size, bn=True):
          up_img = UpSampling2D(size=(2,2), 
                                name = 'DecUpImg'+str(decoder_layer.counter))(img_in)
          up_mask = UpSampling2D(size=(2,2), 
                                name = 'DecUpMsk'+str(decoder_layer.counter))(mask_in)
          concat_img = Concatenate(axis=3, 
                                name = 'DecConcatImg'+str(decoder_layer.counter))([e_conv,up_img])
          concat_mask = Concatenate(axis=3, 
                                name = 'DecConcatMsk'+str(decoder_layer.counter))([e_mask,up_mask])
          conv, mask = PConv2D(filters, kernel_size, strides=1, padding='same', 
                                name = 'DecPConv2D'+str(decoder_layer.counter))([concat_img, concat_mask])
          if bn:
              conv = BatchNormalization(name = 'DecBN'+str(decoder_layer.counter))(conv)
          conv = LeakyReLU(alpha=0.2, 
                           name = 'DecLReLU'+str(decoder_layer.counter))(conv)
          decoder_layer.counter += 1
          return conv, mask
      
      decoder_layer.counter = 0
      ...
      d_conv16, d_mask16 = decoder_layer(d_conv15, d_mask15, inputs_img_P, inputs_mask_P, 3, 3, bn=False)
      outputs_P = Conv2D(3, 1, activation = 'sigmoid', name='outputs_img')(d_conv16)
      
      if np.log2(self.img_rows)-int(np.log2(self.img_rows))!=0 or np.log2(self.img_cols)-int(np.log2(self.img_cols))!=0: # dimensions are not a power of 2
          outputs = Cropping2D(((pad_r1, pad_r2),(pad_c1, pad_c2)), 
                                name = 'OutCrop')(outputs_P)
      else:
          outputs = outputs_P
      if self.c_dim == 1:
          outputs = Lambda(lambda x: tf.reduce_mean(x, axis = -1, keepdims = True), 
                           name = 'OutLambda')(outputs)
  ...
  def assign_proba_center(self, tf_centers, center):
    dist = tf.reduce_sum(tf.abs(tf.tile(
        tf.expand_dims(center,0),
        [len(tf_centers)]+[1]*len(center.shape))-tf_centers),
        [1,2])
    return tf.exp(-dist)#/(tf.reduce_sum(tf.exp(-dist))+non_zero)

  def assign_center(self, tf_centers, center):
    return tf.argmin(tf.reduce_sum(tf.abs(
        tf.tile(tf.expand_dims(center,0),
                [len(tf_centers)]+[1]*len(center.shape))-tf_centers),
        [1,2]))
