...
class NBeats(tf.keras.Model):
  ...
  def compile(self, lr=0.0002):
    super(NBeats, self).compile(metrics = [self.PSNR])
  ...
  def build_pconv_unet(self, train_bn=True):      
    ...
    nbeats_model = NBeatsNet(
            input_dim=self.img_cols, output_dim=self.img_cols,
            backcast_length=self.img_rows-int(self.img_rows*self.mask_ratio), 
            forecast_length=int(self.img_rows*self.mask_ratio),
            stack_types=(NBeatsNet.GENERIC_BLOCK,)*self.n_blocks,
            nb_blocks_per_stack=1, thetas_dim=(4)*self.n_blocks, share_weights_in_stack=True,
            hidden_layer_units=100
        )
    nb_estimator = tf.keras.Model(inputs = nbeats_model.inputs,
                                  outputs = nbeats_model.outputs)
    nbeats_out = nb_estimator(inputs_boolm)
    #
    outputs = tf.concat([inputs_boolm, nbeats_out], axis=1)
