...
class GRU(tf.keras.Model):
  ...
  def build_pconv_unet(self, train_bn=True):      
    ...
    gru_layer = tf.keras.layers.GRU(self.img_cols, return_sequences=True)
    gru_outputs = gru_layer(inputs_boolm)[:,-int(self.img_rows*self.mask_ratio):]
    #
    outputs = tf.concat([inputs_boolm, gru_outputs], axis=1)
