...
# from libs.info3k import NPInfo3_lkk_JoinCCC
...
class NPCenterStat(object):
    ...
    def fit_batch(self, lab, cla, img_batch):
        ...
        lab = np.hstack([[l]*img_batch[i].shape[0] for i,l in enumerate(lab)])
        cla = np.hstack([[c]*img_batch[i].shape[0] for i,c in enumerate(cla)])
        # print("in_batch", img_batch.shape[0])
        # print("in_type", type(img_batch))
        # print("in_shape", np.squeeze(img_batch[0]).shape)
        # print("variant_inshape", np.squeeze(img_batch).shape)
        # print("tcs", to_kcentroid_seq(
        #     np.squeeze(img_batch[0]), k=1))
        # print('\n')
        res = np.hstack(
            [to_kcentroid_seq(
...
class NPJointCenterStat(object):
    ...
    def __init__(self, labels, classes1, classes2, centers0, centers1, centers2, name = 'centers_count'):
        ...
        # self.ibtime_pio = NPInfo3_lkk_JoinCCC(labels, classes1, classes2,
        #                                   name=name+'_info_priorinout')
    ...
    def result(self):
        # 2 dict: p(l,k1,k2,[c0/OR/c1,c2])
        return (self.freqdict_prior.result(), self.freqdict_io.result())
    ...
    def fit_batch(self, lab, cla, img_batch):
        ...
        self.update(lab, cla, res)
        # TODO verifiy res comply with the goal + implement corresponding vclass + plot in test
