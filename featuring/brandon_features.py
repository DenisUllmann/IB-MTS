# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 13:52:19 2019

@author: Brandon Panos, modified by Denis Ullmann
"""
import os
...
class Mg_settings(object):
    def __init__(self):
        self.lambda_min = 2793.8401 #2794.14
        self.lambda_max = 2806.02 #2805.72

spectra_settings = Mg_settings()

def feature_transform(nprof, dn_nprof):
    '''
    ...
    lambda_min = spectra_settings.lambda_min
    lambda_max = spectra_settings.lambda_max
    n_bins = 240
    window_width = 3
    ...
    #----------- tripplet feature ----------------
    #print("5) triplet emission")
#    if os.path.isfile('nan_count.npz'):
#        nan_count = np.load('nan_count.npz', allow_pickle=True)['nan_count'].all()
#    else:
#        nan_count = {}
#    if (nprof[:,wing] <= 0).any():
#        if 'wing' in nan_count.keys():
#            nan_count['wing'] = nan_count['wing'] + np.sum(nprof[:,wing] <= 0)
#        else:
#            nan_count['wing'] = np.sum(nprof[:,wing] == 0)
#    if (nprof[:, sm] <= 0).any():
#        if 'sm' in nan_count.keys():
#            nan_count['sm'] = nan_count['sm']  + np.sum(nprof[:,sm] <= 0)
#        else:
#            nan_count['sm'] = np.sum(nprof[:,sm] <= 0)
#    np.savez('nan_count', nan_count=nan_count)
    trip = np.log(nprof[:, sm]/nprof[:,wing])
    # put nan values to 0 (they are due to negative nprof[:, sm] or nprof[:,wing]))
    #trip[np.isnan(trip)] = 0
    #---------------- k/h ratio --------------------
    #print("6) k/h ratio")
